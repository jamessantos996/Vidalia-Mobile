...
$status           = " ";
    ...
    if ($typeOfLoan == "Personal") {
        ...
        if ($city == "Others") {
            ...
            if ($fname == '' || $mname == '' || $lname == '' || $typeOfLoan == '' || $bdate == '' || $email == '' || $cellphone == '' || $homeAddress == '' || $city == '' || $typeOfResidence == '' || $presentCompany == '' || $jobPosition == '' || $employmentStatus == '' || $currentSalary == '' || $employmentYear == '' || $employmentMonth == '' || $appliedLoanAmount == '') {
                echo '{ "alert": "error", "message": "Email <strong>could not</strong> be sent due to some Unexpected Error. Please Try Again later.<br /><br /><strong>" }';
            } else {
                $sendEmail1 = $mail->Send();
                if ($sendEmail1 == true) {
