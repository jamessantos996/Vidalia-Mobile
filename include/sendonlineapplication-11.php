...
// $bdate = $_POST['template-contactform-birth-date'];
...
if ($agree != "agree") {
    ...
    if ($typeOfLoan == "Personal") {
        ...
        if ($city == "Others") {
            ...
            if ($fname == '' || $mname == '' || $lname == '' || $typeOfLoan == '' || $bdate == '' || $email == '' || $cellphone == '' || $landline == '' || $homeAddress == '' || $zipcode == '' || $city == '' || $typeOfResidence == '' || $ResidenceYear == '' || $ResidenceMonth == '' || $provincialAddress == '' || $presentCompany == '' || $jobPosition == '' || $employmentStatus == '' || $currentSalary == '' || $employmentYear == '' || $employmentMonth == '' || $appliedLoanAmount == '' || $hearAboutUs == '') {
                echo '{ "alert": "error", "message": "Email <strong>could not</strong> be sent due to some Unexpected Error. Please Try Again later.<br /><br /><strong>" }';
            } else {
                $sendEmail1 = $mail->Send();
                if ($sendEmail1 == true) {
