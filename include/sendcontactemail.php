...
$recaptcha_failed='Please click on the reCAPTCHA box';
 ...
 if (isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])) {
     ...
     $emailToSend = '';
     $supportArray = ['inquiry','follow_up','technical_problem','other_loan_application_concern','proposal','complaint','other_concern'];
     if(in_array( $emailsubject,$supportArray)){
        $emailToSend = 'support@vidalia.com.ph';
     }

     $collectionArray = ['payment_concern','proof_of_payment','other_payment_concern'];
     if(in_array( $emailsubject,$collectionArray)){
        $emailToSend = 'collection@vidalia.com.ph';
     }
     ...
     $mail->From = $emailToSend;
     $mail->addReplyTo($email, $name);
     $mail->FromName = 'Vidalia Lending Corp';

     $mail->addAddress("support@vidalia.com.ph", "Vidalia Lending Corporation");     // Add a recipient
...
$mail->Subject = $subject;
     ...
     $forapplicantmail->From = $emailToSend;
    ...
    //  $forapplicantmail->addAddress( $email , "Vidalia Lending Corporation" );     // Add a recipient
     ...
     if (strlen($message) > 150) {
         echo '{ "alert": "error", "message": "Your message is too long. Please shorten your message and try submitting again." }';
        exit();
     } elseif ($name == '' || $email == '' || $phone == '' || $emailsubject == '' || $message == '') {
